...
def epoch(model, data, criterion, optimizer=None):
    ...
    for i, (imgs, flow) in enumerate(data):
        ...
        ep_error, aa_error = evaluate(outputs[0], flow)
        ...
        if i % PRINT_INTERVAL == 0:
            print('[{0:s} Batch {1:03d}/{2:03d}]\t'
                  ...
                  'EPE {epe.val:5.1f} ({epe.avg:5.1f})\t'
                  'AAE {aae.val:5.1f} ({aae.avg:5.1f})'.format(
                   "EVAL" if optimizer is None else "TRAIN", i, len(data), batch_time=avg_batch_time, loss=avg_loss,
                   epe=avg_EPE, aae=avg_AAE))

    print('\n===============> Total time {batch_time:d}s\t'
          'Avg loss {loss.avg:.4f}\t'
          'Avg EPE {epe.avg:5.2f} \t'
          'Avg AAE {aae.avg:5.2f} \n'.format(
...
if __name__ == '__main__':
    ...
    for e in range(starting_epoch, args.epochs):
        ...
        if "Flying" in args.root:
            if e < 10000:
                optim.param_groups[0]['lr'] += (1e-4 - args.lr) / 10000
            elif e % 100000 == 0:
